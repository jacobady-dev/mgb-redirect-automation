"""Core redirect generation engine aligned to the Redirect Mapping workbook.

Workbook-oriented behavior:
- Most crawl rows go to Redirect List for CSV.
- Red Flags are reserved for true data conflicts or malformed cases.
- Manual excludes, when enabled, are ignored completely.
- Blocked/noindex rows are excluded from the final CSV.
"""
...
SOURCE_COLUMN_PATTERNS = ["source url", "source url 1", "source url 2", "source url 3", "existing url", "old url", "address"]
...
FILE_EXTENSIONS = {".pdf", ".doc", ".docx", ".xls", ".xlsx", ".ppt", ".pptx", ".zip", ".csv"}
...
def load_all_sheets(path: Path) -> Dict[str, pd.DataFrame]:
    ...
    for sheet_name in workbook.sheet_names:
        try:
            sheets[sheet_name] = clean_columns(pd.read_excel(path, sheet_name=sheet_name))
...
def is_redirect_status(status_code: str) -> bool:
    return safe_str(status_code).startswith("3")


def is_non_200_non_3xx(status_code: str) -> bool:
    code = safe_str(status_code)
...
def source_columns_for_df(df: pd.DataFrame, configured_candidates: Iterable[str]) -> List[str]:
    ...
    return [col for col in df.columns if safe_str(col).lower() in configured or safe_str(col).lower() in SOURCE_COLUMN_PATTERNS]
...
def build_ia_map(site_root: Path, site_config: Dict[str, Any], rules_config: Dict[str, Any]) -> Tuple[Dict[str, str], List[Dict[str, str]], List[Dict[str, str]]]:
    ...
    source_candidates = ia_rule.get("source_column_candidates", CRAWL_URL_COLUMNS + SOURCE_COLUMN_PATTERNS)
    destination_candidates = ia_rule.get("destination_column_candidates", DESTINATION_COLUMN_CANDIDATES)
                ...
                if destination:
                    if source in mapping and mapping[source] != destination:
                        conflicts.append({
                            ...
                            "829 Notes": f"Conflicting IA destinations. Existing mapped destination: {mapping[source]}. Conflict found on sheet '{sheet_name}'.",
                            "Status Code": "301",
                            "Index Status": "",
                            "Rule ID": "ia_conflicting_destinations",
                        })
                    else:
    ...
    return mapping, incomplete_rows, conflicts
...
def asset_destination(asset_type: str, rules_config: Dict[str, Any]) -> str:
    ...
    defaults = {
        "image": "Image Subfolder (TBD)",
        "video": "Video Subfolder (TBD)",
        "file": "File Subfolder (TBD)",
    }
...
def process_site(site_id: str) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    ia_map, incomplete_ia_rows, ia_conflicts = build_ia_map(site_root, site_config, rules_config)
    ...
    for _, row in crawl_df.iterrows():
        ...
        if normalized_url in incomplete_ia_lookup:
            row_note = incomplete_ia_lookup[normalized_url]
    ...
    # Add IA conflicts not encountered in the crawl for visibility.
    ...
    for key, df in [("redirect_list", redirect_df), ("red_flags", red_flag_df), ("ignored_search_agency", ignored_df), ("qa_summary", qa_df)]:
    ...
    if internal_relative:
        ...
        pd.concat([redirect_df_internal.assign(Output="Redirect List"), red_flag_df_internal.assign(Output="Red Flags")], ignore_index=True).to_csv(internal_path, index=False)
